...
Download the data from:[http://bit.ly/2evcR9I](http://bit.ly/2evcR9I)
...
1. Within Visual Studio, open “Create Table- In Memory.sql"
2. Adjust the path to the DataPoint.bcp file so it matches the location of your project and save the script.
3. Execute the script to load each table with 16 million rows worth of sample data.
...
3. When the script completes, observe that 1383 rows were returned.Take note of how long the query took to execute.

4. Now, execute the script to summarize the time series data stored in the memory-optimized table.
When the script completes, observe that 1383 rows were returned.Take note of how long the query took to execute.

You should notice that the performance of the query against the memory-optimized table runs between 2x-10x faster than the same query, running against the same data stored in a disk based table.
...
8. Observe that more than 1383 rows were returned.Take note of how long the query took to execute.
...
15. Observe that more than 1383 rows were returned.Take note of how long the query took to execute.
